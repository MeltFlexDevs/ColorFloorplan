...
def build_windows(window_polygons: "list[Polygon]", rooms: "list[Polygon]", room_tree: STRtree | None, builder: MeshBuilder, height: float = 2.6):
    """Build window meshes: combined parapet+lintel (for frontend bounding box) + face meshes.
    ...
    Window top is EXPLICITLY tied to door top so they always match:
      ...
      - Window:     0.8m to DOOR_TOP  (opening, glass area)
      - Lintel:     DOOR_TOP to height (wall above window = wall above door)
    """
    DOOR_TOP  = height * 10 / 13      # MUST match build_doors: door opening height
    PARAPET_H = 0.8                    # fixed sill height (standard ~80cm)
    WINDOW_H  = DOOR_TOP - PARAPET_H  # window opening fills from parapet to door top
    LINTEL_H  = height - DOOR_TOP     # lintel = same as wall above door
    ...
    for poly in window_polygons:
        ...
        # Window_N: Combined parapet (bottom) + lintel (top) in ONE mesh.
        # The frontend reads the bounding box (Y: 0 to height) to get dimensions,
        # then hides this mesh and adds only frame + glass in the opening.
        builder.extrude_shape(indices, verts, height=PARAPET_H)                              # parapet: 0 to 0.8
        builder.extrude_shape(indices, verts, height=LINTEL_H, offset=PARAPET_H + WINDOW_H)  # lintel:  2.0 to 2.6
        ...
        # Wall above window (lintel) — decomposed into 6 face meshes
        build_element_faces(poly, rooms, room_tree, builder, "WindowTop", win_num, LINTEL_H, PARAPET_H + WINDOW_H)
